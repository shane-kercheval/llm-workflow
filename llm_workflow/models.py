...
from llm_workflow.internal_utilities import (
    calculate_num_tokens_hugging_face,
...
def llama_message_formatter(
    ...
    formatted_messages = ""
    if system_message:
        formatted_messages += f"[INST] <<SYS>> {system_message} <</SYS>> [/INST]\n"
    for message in messages:
        formatted_messages += f"[INST] {message.prompt} [/INST]\n"
        formatted_messages += f"{message.response}\n"
    if prompt:
        formatted_messages += f"[INST] {prompt} [/INST]\n"
...
class HuggingFaceEndpointChat(PromptModel):
    ...
    def _run(self, prompt: str) -> ExchangeRecord:
        ...
        messages = self._message_formatter(self._system_message, history, prompt)
        response = ""
        start = time.time()
        while True:
            ...
            output = retry_handler()(
                ...
                payload={
                    ...
                    "parameters": {
                        'max_new_tokens': self.max_streaming_tokens,  # controls chunk/delta size
                },
            })
        ...
        self._previous_messages = messages
        prompt_tokens = calculate_num_tokens_hugging_face(
            value=prompt,
            tokenizer=self.tokenizer,
        )
        completion_tokens = calculate_num_tokens_hugging_face(
            value=response,
            tokenizer=self.tokenizer,
        )
        total_tokens = prompt_tokens + completion_tokens
